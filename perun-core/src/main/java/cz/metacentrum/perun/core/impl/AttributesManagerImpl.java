...
public class AttributesManagerImpl implements AttributesManagerImplApi {
	...
	protected static class AttributeHoldersRowMapper implements RowMapper<AttributeHolders> {
		...
		public AttributeHolders mapRow(ResultSet rs, int i) throws SQLException {
			Attribute attribute = new Attribute(ATTRIBUTE_MAPPER.mapRow(rs, i));
			...
			try {
				if(this.primaryHolderType == null) return new AttributeHolders(attribute, rs.getString("subject"), 1);
				else {
					...
					return new AttributeHolders(attribute, primaryHolder, secondaryHolder, 1);
				}
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Facility facility) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.FACILITY);
			return this.setValuesOfAttributes(sess, attrs, facility, null);
		}
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Member member) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.MEMBER);
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Vo vo) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.VO);
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Group group) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.GROUP);
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Host host) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.HOST);
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, Resource resource) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.RESOURCE);
			return this.setValuesOfAttributes(sess, attrs, resource, null);
		}
	...
	public List<Attribute> getAttributes(PerunSession sess, Member member, Group group, List<String> attrNames) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getAttributesByNames(attrNames, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
	...
	public List<Attribute> getAttributes(PerunSession sess, Vo vo, List<String> attrNames) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getAttributesByNames(attrNames, new Holder(vo.getId(), Holder.HolderType.VO));
			return this.setValuesOfAttributes(sess, attrs, vo, null);
		}
	...
	public List<Attribute> getAttributes(PerunSession sess, Member member, List<String> attrNames) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getAttributesByNames(attrNames, new Holder(member.getId(), Holder.HolderType.MEMBER));
			return this.setValuesOfAttributes(sess, attrs, member, null);
		}
	...
	public List<Attribute> getAttributes(PerunSession sess, Group group, List<String> attrNames) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getAttributesByNames(attrNames, new Holder(group.getId(), Holder.HolderType.GROUP));
			return this.setValuesOfAttributes(sess, attrs, group, null);
		}
	...
	public List<Attribute> getAttributes(PerunSession sess, User user, List<String> attrNames) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getAttributesByNames(attrNames, new Holder(user.getId(), Holder.HolderType.USER));
			return this.setValuesOfAttributes(sess, attrs, user, null);
		}
	...
	public List<Attribute> getVirtualAttributes(PerunSession sess, User user) throws InternalErrorException {
		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) {
			List<Attribute> attrs = perun.getCacheManager().getVirtualAttributes(Holder.HolderType.USER);
			return this.setValuesOfAttributes(sess, attrs, user, null);
		}
	...
	public Attribute getAttribute(PerunSession sess, Facility facility, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(facility.getId(), Holder.HolderType.FACILITY));
...
//		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) return perun.getCacheManager().getAllSimilarAttributeNames(startingPartOfAttributeName);
	...
	public Attribute getAttribute(PerunSession sess, Vo vo, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(vo.getId(), Holder.HolderType.VO));
	...
	public Attribute getAttribute(PerunSession sess, Group group, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(group.getId(), Holder.HolderType.GROUP));
	...
	public Attribute getAttribute(PerunSession sess, Resource resource, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
	...
	public Attribute getAttribute(PerunSession sess, Member member, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(member.getId(), Holder.HolderType.MEMBER));
	...
	public Attribute getAttribute(PerunSession sess, User user, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(user.getId(), Holder.HolderType.USER));
	...
	public Attribute getAttribute(PerunSession sess, Host host, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeByName(attributeName, new Holder(host.getId(), Holder.HolderType.HOST));
	...
	public Attribute getAttribute(PerunSession sess, String key, String attributeName) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getEntitylessAttributeByNameAndKey(key, attributeName);
...
//		if(!CacheManager.isCacheDisabled() && !perun.getCacheManager().wasCacheUpdatedInTransaction()) return perun.getCacheManager().getAttributesDefinitions();
	...
	public AttributeDefinition getAttributeDefinitionById(PerunSession sess, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) return perun.getCacheManager().getAttributeDefinitionById(id);
	...
	public Attribute getAttributeById(PerunSession sess, Facility facility, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(facility.getId(), Holder.HolderType.FACILITY));
			return setValueOfAttribute(sess, attr, facility, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, Vo vo, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(vo.getId(), Holder.HolderType.VO));
			return setValueOfAttribute(sess, attr, vo, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, Resource resource, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
			return setValueOfAttribute(sess, attr, resource, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, Group group, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(group.getId(), Holder.HolderType.GROUP));
			return setValueOfAttribute(sess, attr, group, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, Host host, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(host.getId(), Holder.HolderType.HOST));
			return setValueOfAttribute(sess, attr, host, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, Member member, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(member.getId(), Holder.HolderType.MEMBER));
			return setValueOfAttribute(sess, attr, member, null);
		}
	...
	public Attribute getAttributeById(PerunSession sess, User user, int id) throws InternalErrorException, AttributeNotExistsException {
		if(!CacheManager.isCacheDisabled()) {
			Attribute attr = perun.getCacheManager().getAttributeById(id, new Holder(user.getId(), Holder.HolderType.USER));
			return setValueOfAttribute(sess, attr, user, null);
		}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY));
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							this.setAttributeCreatedAndModified(sess, attribute);
							if (0 < jdbc.execute("merge into facility_attr_values using dual on (attr_id=? and facility_id=?) " +
									...
									"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)" +
									"when matched       then update set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt(),
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into facility_attr_values (attr_id, facility_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), facility.getId(),
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update facility_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and facility_id=?",
					...
					try {
						if(Compatibility.isMergeSupported()) {
							...
							jdbc.update("merge into facility_attr_values using dual on (attr_id=? and facility_id=?) " +
									...
									"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?) when matched then update set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt(),
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update facility_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and facility_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), facility.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(vo.getId(), Holder.HolderType.VO));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into vo_attr_values using dual on (attr_id=? and vo_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(vo.getId(), Holder.HolderType.VO));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into vo_attr_values (attr_id, vo_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), vo.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(vo.getId(), Holder.HolderType.VO));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update vo_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and vo_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update vo_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and vo_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), vo.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(vo.getId(), Holder.HolderType.VO));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(host.getId(), Holder.HolderType.HOST));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into host_attr_values using dual on (attr_id=? and host_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(host.getId(), Holder.HolderType.HOST));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into host_attr_values (attr_id, host_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), host.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(host.getId(), Holder.HolderType.HOST));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update host_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and host_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update host_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and host_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), host.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(host.getId(), Holder.HolderType.HOST));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into group_attr_values using dual on (attr_id=? and group_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(group.getId(), Holder.HolderType.GROUP));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into group_attr_values (attr_id, group_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), group.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update group_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and group_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update group_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and group_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), group.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(group.getId(), Holder.HolderType.GROUP));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into resource_attr_values using dual on (attr_id=? and resource_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into resource_attr_values (attr_id, resource_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), resource.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update resource_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=?",
					...
					try {
						if(Compatibility.isMergeSupported()) {
							...
							jdbc.update("merge into resource_attr_values using dual on (attr_id=? and resource_id=?) " +
									...
									"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)" +
									"when matched       then update set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt(),
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update resource_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), resource.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into member_resource_attr_values using dual on (attr_id=? and resource_id=? and member_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							return false;
						} else {
							try {
								...
								this.setAttributeCreatedAndModified(sess, attribute);
								//Value doesn't exist -> insert   (and return from this metod)
								jdbc.update("insert into member_resource_attr_values (attr_id, resource_id, member_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)",attribute.getId(), resource.getId(),
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_resource_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=? and member_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							this.setAttributeCreatedAndModified(sess, attribute);
							jdbc.update("merge into member_resource_attr_values using dual on (attr_id=? and resource_id=? and member_id=?) " +
							...
							try {
								...
								this.setAttributeCreatedAndModified(sess, attribute);
								//Value doesn't exist -> insert   (and return from this metod)
								jdbc.update("insert into member_resource_attr_values (attr_id, resource_id, member_id, attr_value, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)",attribute.getId(), resource.getId(),
										member.getId(), BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_resource_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=? and member_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), resource.getId(), member.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							this.setAttributeCreatedAndModified(sess, attribute);
							if (0 < jdbc.execute("merge into member_group_attr_values using dual on (attr_id=? and group_id=? and member_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
								return true;
							}
							return false;
						} else {
							try {
								...
								this.setAttributeCreatedAndModified(sess, attribute);
								//Value doesn't exist -> insert (and return from this method
								jdbc.update("insert into member_group_attr_values (attr_id, group_id, member_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
												"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), group.getId(),
										member.getId(), BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_group_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and group_id=? and member_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							this.setAttributeCreatedAndModified(sess, attribute);
							jdbc.update("merge into member_group_attr_values using dual on (attr_id=? and group_id=? and member_id=?) " +
							...
							try {
								...
								this.setAttributeCreatedAndModified(sess, attribute);
								//Value doesn't exist -> insert (and return from this metod)
								jdbc.update("insert into member_group_attr_values (attr_id, group_id, member_id, attr_value, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
												"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), group.getId(),
										member.getId(), BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
								return true;
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_group_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and group_id=? and member_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), group.getId(), member.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER), new Holder(facility.getId(), Holder.HolderType.FACILITY));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into user_facility_attr_values using dual on (attr_id=? and facility_id=? and user_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(user.getId(), Holder.HolderType.USER), new Holder(facility.getId(), Holder.HolderType.FACILITY));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into user_facility_attr_values (attr_id, facility_id, user_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), facility.getId(), user.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER), new Holder(facility.getId(), Holder.HolderType.FACILITY));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update user_facility_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and facility_id=? and user_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update user_facility_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and facility_id=? and user_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), facility.getId(), user.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(user.getId(), Holder.HolderType.USER), new Holder(facility.getId(), Holder.HolderType.FACILITY));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into user_attr_values using dual on (attr_id=? and user_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(user.getId(), Holder.HolderType.USER));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into user_attr_values (attr_id, user_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), user.getId(), BeansUtils.attributeValueToString(attribute),
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update user_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and user_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update user_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and user_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), user.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(user.getId(), Holder.HolderType.USER));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into member_attr_values using dual on (attr_id=? and member_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into member_attr_values (attr_id, member_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), member.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and member_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and member_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), member.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into group_resource_attr_values using dual on (attr_id=? and resource_id=? and group_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into group_resource_attr_values (attr_id, resource_id, group_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										"values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), resource.getId(), group.getId(),
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update group_resource_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=? and group_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update group_resource_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and resource_id=? and group_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), resource.getId(), group.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
							return true;
						}
	...
	private boolean setAttributeWithNullValue(final PerunSession sess, final String key, final Attribute attribute) throws InternalErrorException {
		try {
			jdbc.update("insert into entityless_attr_values (attr_id, subject, attr_value, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) "
									+ "values (?,?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), key, null, null,
									sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
			return true;
		} catch (RuntimeException ex) {
			throw new InternalErrorException(ex);
		...
		try {
			if (isLargeAttribute(sess, attribute)) {
				if (attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeEntitylessAttribute(attribute, key);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while (true) {
					try {
						if (Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into entityless_attr_values using dual on (attr_id=? and subject=?) "
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setEntitylessAttributeWithExistenceCheck(attribute, key);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into entityless_attr_values (attr_id, subject, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) "
										+ "values (?,?,?,?,?," + attribute.getValueCreatedAt() + "," + attribute.getValueModifiedAt() + ",?,?)", attribute.getId(), key, BeansUtils.attributeValueToString(attribute),
										sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setEntitylessAttribute(attribute, key);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update entityless_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and subject=?",
				...
				while (true) {
					try {
						if (Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update entityless_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + attribute.getValueModifiedAt() + " where attr_id=? and subject=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), key);
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setEntitylessAttributeWithExistenceCheck(attribute, key);
							return true;
						}
	...
	public AttributeDefinition createAttribute(PerunSession sess, AttributeDefinition attribute) throws InternalErrorException, AttributeExistsException {
		...
		try {
			...
			jdbc.update("insert into attr_names (id, attr_name, type, dsc, namespace, friendly_name, display_name, created_by, created_at, modified_by, modified_at, created_by_uid, modified_by_uid) " +
					...
					sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), attribute.getModifiedAt(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
	...
	public void deleteAttribute(PerunSession sess, AttributeDefinition attribute) throws InternalErrorException {
		...
		try {
			...
			if(!CacheManager.isCacheDisabled()) perun.getCacheManager().reinitializeCache(sess, this);
	...
	public void removeAllGroupResourceAttributes(PerunSession sess, Resource resource) throws InternalErrorException {
		try {
			...
			if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAllAttributesBySecondaryHolder(Holder.HolderType.GROUP, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
		} catch (RuntimeException ex) {
			throw new InternalErrorException(ex);
		}
	}
	...
	public boolean removeAttribute(PerunSession sess, Facility facility, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from facility_attr_values where attr_id=? and facility_id=?", attribute.getId(), facility.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY));
	...
	public boolean removeAttribute(PerunSession sess, Vo vo, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from vo_attr_values where attr_id=? and vo_id=?", attribute.getId(), vo.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(vo.getId(), Holder.HolderType.VO));
	...
	public boolean removeAttribute(PerunSession sess, Group group, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from group_attr_values where attr_id=? and group_id=?", attribute.getId(), group.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP));
	...
	public boolean removeAttribute(PerunSession sess, Resource resource, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from resource_attr_values where attr_id=? and resource_id=?", attribute.getId(), resource.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE));
	...
	public boolean removeAttribute(PerunSession sess, Member member, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from member_attr_values where attr_id=? and member_id=?", attribute.getId(), member.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER));
	...
	public void removeAllUserFacilityAttributesForAnyUser(PerunSession sess, Facility facility) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from user_facility_attr_values where facility_id=?", facility.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAllAttributesBySecondaryHolder(Holder.HolderType.USER, new Holder(facility.getId(), Holder.HolderType.FACILITY));
	...
	public void removeAllUserFacilityAttributes(PerunSession sess, User user) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from user_facility_attr_values where user_id=?", user.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAllAttributesByPrimaryHolder(new Holder(user.getId(), Holder.HolderType.USER), Holder.HolderType.FACILITY);
	...
	public boolean removeAttribute(PerunSession sess, User user, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from user_attr_values where attr_id=? and user_id=?", attribute.getId(), user.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER));
	...
	public boolean removeAttribute(PerunSession sess, Host host, AttributeDefinition attribute) throws InternalErrorException {
		try {
			if(0 < jdbc.update("delete from host_attr_values where attr_id=? and host_id=?", attribute.getId(), host.getId())) {
				if (!CacheManager.isCacheDisabled()) perun.getCacheManager().removeAttribute(attribute, new Holder(host.getId(), Holder.HolderType.HOST));
	...
	public AttributeDefinition updateAttributeDefinition(PerunSession perunSession, AttributeDefinition attributeDefinition) throws InternalErrorException {
		try {
			...
			if (!attributeDefinition.getDescription().equals(map.get("dsc"))) {
				...
				jdbc.update("update attr_names set dsc=?, modified_by=?, modified_by_uid=?, modified_at=" + attributeDefinition.getModifiedAt() + "  where id=?", attributeDefinition.getDescription(), perunSession.getPerunPrincipal().getActor(), perunSession.getPerunPrincipal().getUserId(), attributeDefinition.getId());
			...
			if ((map.get("display_name") == null &&  attributeDefinition.getDisplayName() != null) ||
				...
				this.setAttributeDefinitionModified(perunSession, attributeDefinition);
				jdbc.update("update attr_names set display_name=?, modified_by=?, modified_by_uid=?, modified_at=" + attributeDefinition.getModifiedAt() + "  where id=?", attributeDefinition.getDisplayName(), perunSession.getPerunPrincipal().getActor(), perunSession.getPerunPrincipal().getUserId(), attributeDefinition.getId());
	...
	private Attribute setAttributeCreatedAndModified(PerunSession sess, Attribute attribute) throws InternalErrorException {
		...
		//TODO cache created by uid?? neexistuje v kode, len v tabulke
//		attribute.setCreatedByUid(sess.getPerunPrincipal().getUserId());
//		attribute.setModifiedByUid(sess.getPerunPrincipal().getUserId());
		String time = jdbc.queryForObject("SELECT " + Compatibility.getSysdate(), String.class);
	...
	private Attribute setAttributeModified(PerunSession sess, Attribute attribute) throws InternalErrorException {
		attribute.setValueModifiedBy(sess.getPerunPrincipal().getActor());
		attribute.setValueModifiedAt(jdbc.queryForObject("SELECT " + Compatibility.getSysdate(), String.class));
		return attribute;
	}
	...
	private AttributeDefinition setAttributeDefinitionCreatedAndModified(PerunSession sess, AttributeDefinition attribute) throws InternalErrorException {
		...
		attribute.setModifiedByUid(sess.getPerunPrincipal().getUserId());
		String time = jdbc.queryForObject("SELECT " + Compatibility.getSysdate(), String.class);
		attribute.setCreatedAt(time);
		attribute.setModifiedAt(time);
		return attribute;
	}
	...
	private AttributeDefinition setAttributeDefinitionModified(PerunSession sess, AttributeDefinition attribute) throws InternalErrorException {
		attribute.setModifiedBy(sess.getPerunPrincipal().getActor());
		attribute.setModifiedByUid(sess.getPerunPrincipal().getUserId());
		attribute.setModifiedAt(jdbc.queryForObject("SELECT " + Compatibility.getSysdate(), String.class));
		return attribute;
	}
}
