		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY), null);
					...
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into facility_attr_values using dual on (attr_id=? and facility_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(facility.getId(), Holder.HolderType.FACILITY), null);
								return true;
							}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(vo.getId(), Holder.HolderType.VO), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into vo_attr_values using dual on (attr_id=? and vo_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(vo.getId(), Holder.HolderType.VO), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into vo_attr_values (attr_id, vo_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										...
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(vo.getId(), Holder.HolderType.VO), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(host.getId(), Holder.HolderType.HOST), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into host_attr_values using dual on (attr_id=? and host_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(host.getId(), Holder.HolderType.HOST), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into host_attr_values (attr_id, host_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										...
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(host.getId(), Holder.HolderType.HOST), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into group_attr_values using dual on (attr_id=? and group_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into group_attr_values (attr_id, group_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										...
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(group.getId(), Holder.HolderType.GROUP), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into resource_attr_values using dual on (attr_id=? and resource_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into resource_attr_values (attr_id, resource_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										...
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(resource.getId(), Holder.HolderType.RESOURCE), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_resource_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + Compatibility.getSysdate()+ " where attr_id=? and resource_id=? and member_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_resource_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + Compatibility.getSysdate() + " where attr_id=? and resource_id=? and member_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), resource.getId(), member.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(resource.getId(), Holder.HolderType.RESOURCE));
							return true;
						}
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_group_attr_values set attr_value_text=?, modified_by=?, modified_by_uid=?, modified_at=" + Compatibility.getSysdate() + " where attr_id=? and group_id=? and member_id=?",
				...
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
							jdbc.update("update member_group_attr_values set attr_value=?, modified_by=?, modified_by_uid=?, modified_at=" + Compatibility.getSysdate() + " where attr_id=? and group_id=? and member_id=?",
									BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), attribute.getId(), group.getId(), member.getId());
							if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), new Holder(group.getId(), Holder.HolderType.GROUP));
							return true;
						}
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into user_attr_values using dual on (attr_id=? and user_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(user.getId(), Holder.HolderType.USER), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(user.getId(), Holder.HolderType.USER), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
		...
		try {
			if(isLargeAttribute(sess, attribute)) {
				if(attribute.getValue() == null) {
					...
					if(!CacheManager.isCacheDisabled() && numAffected == 1) perun.getCacheManager().removeAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), null);
					return numAffected == 1;
				}
				int repetatCounter = 0;
				while(true) {
					try {
						if(Compatibility.isMergeSupported()) {
							...
							if (0 < jdbc.execute("merge into member_attr_values using dual on (attr_id=? and member_id=?) " +
								...
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttributeWithExistenceCheck(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), null);
								return true;
							}
							return false;
						} else {
							try {
								...
								jdbc.update("insert into member_attr_values (attr_id, member_id, attr_value_text, created_by, modified_by, created_at, modified_at, created_by_uid, modified_by_uid) " +
										...
										BeansUtils.attributeValueToString(attribute), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getActor(), sess.getPerunPrincipal().getUserId(), sess.getPerunPrincipal().getUserId());
								if(!CacheManager.isCacheDisabled()) perun.getCacheManager().setAttribute(attribute, new Holder(member.getId(), Holder.HolderType.MEMBER), null);
								return true;
							}
							this.setAttributeModified(sess, attribute);
							//Exception wasn't thrown -> update
	...
	public List<Attribute> getRequiredAttributes(PerunSession sess, Service service, Resource resource, Member member) throws InternalErrorException {
		...
		try {
			return jdbc.query("select " + getAttributeMappingSelectQuery("mem") + " from attr_names " +
					...
					"left join   member_resource_attr_values mem    on id=mem.attr_id and mem.resource_id=? and member_id=? " +
					"where namespace in (?,?,?)",
					new AttributeRowMapper(sess, this, member, resource), service.getId(), resource.getId(), member.getId(), AttributesManager.NS_MEMBER_RESOURCE_ATTR_DEF, AttributesManager.NS_MEMBER_RESOURCE_ATTR_OPT, AttributesManager.NS_MEMBER_RESOURCE_ATTR_VIRT);
		} catch(RuntimeException ex) {
			throw new InternalErrorException(ex);
		}
	}
